...
namespace Footballista.Wasm.Client.Infra.ClientServices
{
    public class GameClientService : IGameService
    {
        ...
        public async Task StartNewCareerAsync(string careerName, Club club, Competition competition, Manager manager)
        {
            ...
            HttpResponseMessage response = await _httpClient.PostAsJsonAsync("career/create", _mapper.Map<CareerDto>(career));
            ...
            if (!Guid.TryParse(responseStr.Replace("\"", ""), out Guid careerId))
            {
                throw new ApplicationException("Cannot parse guid " + responseStr);
            }

            career.DefineId(careerId);
        ...
        public void ExitCareer() => _localStorageService.RemoveItem(LocalStorageKeys.CURRENT_CAREER);
    }
}
