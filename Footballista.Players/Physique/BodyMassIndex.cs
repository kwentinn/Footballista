...
namespace Footballista.Players.Physique
{
	...
	public class BodyMassIndex : ValueObject
	{
		...
		public double BMI 
			=> Math.Round(Weight.Kilograms / Math.Pow(Height.Meters, 2), 2);
		public BmiRange BmiRange 
			=> _bmiRanges.FirstOrDefault(br => br.Range.IsValueInRange(BMI));
		public BmiType BmiType 
			=> BmiRange.BmiType;
		...
		private static ReadOnlyCollection<BmiRange> _bmiRanges = new List<BmiRange> 
		{ 
